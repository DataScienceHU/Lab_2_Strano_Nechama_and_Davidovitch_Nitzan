...
```{r}
#Creating table for probability's according to each letter
alphabet <- letters[1:26]
...
(sort(table(freq_prb_table), decreasing = TRUE)[1:10])
...
probabilities = function (tablegiven,array) {
  ...
  #freq_prb_table = as.table(setNames(array,final_pro))
    }}
  return (probability)

}
#word_array = c("frozen","skull","winds","worse","there","where")
probabilities(letter_freq_un2,unique(five_letters))
