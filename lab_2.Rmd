...
Compute the distribution of lengths of words you got, and plot using a bar-plot. What are the `median`, `mean`, `longest` and `most common` word lengths?
...
plot(chapter_index,word_amount(chapters))
...
Because the chance of 2 people to choose the same word is iid the chance is the multypication of the distribution of the two words (which is the same)
...
The Geomertrical distribution
$$Pr(X=k)=(1-p)^{k-1}p$$
Cumulitive Geomertrical distribution
$$Pr(X ≤ x) = 1 - (1 - p)^{x}$$
Because each letter is iid the intersection is the mulipication of each letters geometric distribution 
$$Pr(Number of tries <= amount) = Pr(X ≤ x)∩Pr(X ≤ x) ... ∩Pr(X ≤ x) = Pr(X ≤ x)*Pr(X ≤ x) ... *Pr(X ≤ x)$$
...
A function that will calculate the distribution of P(X=x) 
...
new_word = sample(words_string,1)
replicate(100,strat2(words_string,new_word))
